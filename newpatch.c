 ...
 * NVRAM unlock, image type bypass, rootfs, panic, bootx, serial, USB backdoor.
 */
...
#include <stdbool.h>
#include <inttypes.h>
extern addr_t step64_back(const uint8_t *buf, addr_t start, size_t length, uint32_t what, uint32_t mask);
extern addr_t step64(const uint8_t *buf, addr_t start, size_t length, uint32_t what, uint32_t mask);
extern addr_t follow_call64(const uint8_t *buf, addr_t call);
...
/* -------------------------------------------------------------------------
   1. RSA signature check – with scoring selection
   ------------------------------------------------------------------------- */
...
static int find_image4_callback_candidates(uint8_t *buf, size_t len, candidate_t *cands, int max_cands) {
    ...
    for (addr_t off = 0; off < len - 4; off += 4) {
        ...
        cands[count].extra = (void*)(uintptr_t)mov;
        count++;
...
int patch_ctrr_lockdown(struct iboot64_img* iboot_in) {
    ...
    if (count == 0) {
        WARN("No CTRR MSR found\n");
    } else {
        LOG("CTRR lockdown: %d MSR(s) NOPed\n", count);
    }
    return count;
}
...
int enable_kernel_debug(struct iboot64_img* iboot_in) {
    ...
    if (!debug) {
        WARN("debug-enabled not found\n");
        return -1;
    }
    uint64_t xref = iboot64_ref(iboot_in, debug);
    if (xref == (uint64_t)-1) {
        WARN("debug-enabled xref not found\n");
        return -1;
    }
...
int rsa_sigcheck_patch(struct iboot64_img* iboot_in, bool pac) {
    ...
    if (!img4) {
        WARN("IMG4 string not found\n");
        return -1;
    }
    uint64_t img4ref = iboot64_ref(iboot_in, img4);
    if (img4ref == (uint64_t)-1) {
        WARN("IMG4 xref not found\n");
        return -1;
    }
...
/* ======================================================================
   9. ROOTFS BYPASS (LLB) – with scoring selection
   ====================================================================== */
...
int patch_rootfs_bypass(struct iboot64_img* iboot_in) {
    ...
    int bhs_sites[64], bhs_count = 0;
    for (addr_t off = 0; off < len - 8; off += 4) {
        uint32_t cmp = *(uint32_t*)(buf + off);
        if (cmp == 0xF1001D1F) {
            uint32_t next = *(uint32_t*)(buf + off + 4);
            if ((next & 0xFC000000) == 0x54000000) {
...
/* ======================================================================
   10. PANIC BYPASS (LLB) – with scoring selection
   ====================================================================== */
...
/* ======================================================================
   11. BOOTX PRECONDITION (iBEC) – with scoring selection
   ====================================================================== */
...
int patch_bootx_precondition(struct iboot64_img* iboot_in) {
    ...
    // Enumerate hash getters: MOV/MOVK/MOVK/MOVK/RET
    int hash_getters[64], hg_n = 0;
    for (addr_t off = 0; off < len - 20; off += 4) {
        if (*(uint32_t*)(buf + off + 16) != RET_U32) continue;
        int rd = -1, ok = 1;
        for (int k = 0; k < 4; k++) {
    ...
    // Enumerate bit getters: ADRP ; LDRB ; UBFX ; RET
    ...
    for (addr_t off = 0; off < len - 16; off += 4) {
        ...
        if ((i1 & 0xFFC00000) != 0x39400000) continue;
        if ((i2 & 0xFFE00000) != 0x13800000) continue;
    ...
    for (addr_t off = 0; off < len - 4; off += 4) {
        ...
        if ((tbz & 0x7F00001F) != 0x36000000) continue;
...
/* ======================================================================
   13. DYNAMIC USB BACKDOOR (no hardcoded offsets)
   ====================================================================== */
...
static int find_usb_offsets(struct iboot64_img* iboot_in) {
    ...
    if (platform_demote) {
        LOG("platform_demote at 0x%llx\n", (uint64_t)platform_demote);
    } else {
        WARN("platform_demote not found\n");
    }
    ...
    if (platform_set_remote_boot) {
        LOG("platform_set_remote_boot at 0x%llx\n", (uint64_t)platform_set_remote_boot);
    } else {
        WARN("platform_set_remote_boot not found\n");
    }
    ...
    if (aes_crypto_cmd) {
        LOG("aes_crypto_cmd at 0x%llx\n", (uint64_t)aes_crypto_cmd);
    } else {
        WARN("aes_crypto_cmd not found\n");
    }
    ...
    if (jump_away) {
        LOG("jump_away at 0x%llx\n", jump_away);
    } else {
        WARN("jump_away not found\n");
    }
    ...
    if (main_task_stack_lr) {
        LOG("main_task_stack_lr ~ 0x%llx\n", main_task_stack_lr);
    } else {
        WARN("main_task_stack_lr not found\n");
    }
